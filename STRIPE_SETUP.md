# Stripe Setup â€?VIP Subscriptions
...
> How the money flows: user clicks **Upgrade** â†?Stripe's hosted checkout page
> (card entered there, never on our server) â†?Stripe charges the card â†?money
> lands in **your Stripe balance** â†?Stripe **auto-pays out to your bank** on a
> schedule â†?Stripe pings our `/api/stripe/webhook` â†?we flip the user to Pro.
...
2. You can build & test everything in **Test mode** (toggle top-right) before
   activating your account. Test mode uses fake cards â€?no real money.
3. To receive real payouts later, complete **Activate account** (business
   details + your bank account). This is the "connect to my wallet" step â€?your
   bank account *is* the wallet. Do this yourself; never share bank details with
   anyone, including me.

## 2. Create the Product and Prices

1. Dashboard â†?**Product catalog** â†?**+ Add product**.
2. Name: `InterviewMirror Pro`. Add two **recurring** prices:
   - **Monthly** â€?e.g. `$9.00 USD`, billing period **Monthly**.
   - **Yearly** â€?e.g. `$72.00 USD`, billing period **Yearly**.
...
Paste them into `.env.local` (local) and into Vercel's env vars (production â€?
see `DEPLOYMENT.md`).
...
1. With the app + `stripe listen` running, go to `/dashboard/billing` â†?
   **Upgrade to Pro**.
...
4. Click **Manage billing** â†?cancel â†?confirm the badge disappears.
...
2. Switch the dashboard to **Live mode** and redo: create the product/prices,
   ...
   add a **live** webhook endpoint â†?live `whsec_...`.
...
- `lib/stripe/actions.ts` â€?creates the Checkout Session and Billing Portal
  session (server-side; secret key never reaches the browser).
- `app/api/stripe/webhook/route.ts` â€?verifies Stripe's signature and updates
  the user's `profiles` row via the Supabase service-role key.
- `lib/subscription/queries.ts` â€?`isVip()` / `getSubscription()` for gating.
